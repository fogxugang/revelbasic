...
func AppInit() {
    var err error
    // Read configuration.
    Dial = revel.Config.StringDefault("revelbasic.dial", "localhost")
    Method = revel.Config.StringDefault("revelbasic.method", "clone")
...
func MethodError(m string) error {
    ...
    return fmt.Errorf("revelbasic: Invalid session instantiation method '%s'", m)
}
